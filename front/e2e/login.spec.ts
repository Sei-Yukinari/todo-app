...
test('shows login buttons and /api/auth/session returns 400 for missing token', async ({ page, request }) => {
  await page.goto('http://localhost:3000/(auth)/login');
  await expect(page.getByText('Continue with Google')).toBeVisible();
  await expect(page.getByText('Continue with GitHub')).toBeVisible();

  // call front BFF which forwards to backend
  const res = await request.post('http://localhost:3000/api/auth/session', { data: { idToken: 'fake' } });
